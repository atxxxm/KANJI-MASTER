...
pub fn render(ui: &mut egui::Ui, search_query: &mut String, ctx: &AppContext) -> Option<(TabType, TabOpenMode)> {
                    ...
                    ui.horizontal(|ui| {
                        ...
                        for i in start_index..end_index {
                            if let Some(item) = filtered_kanji.get(i) {
                                ...
                                if response.clicked() {
                                    let new_content = crate::ui::views::kanji_detail::create_tab((*item).clone(), ctx.paths);
