...
- **Kanji detail** — stroke-order animation drawn live on canvas, on'yomi/kun'yomi readings, meaning, and usage examples with clickable kanji; click an example to reveal its translation inline.
...
- **Interface language** — the whole UI (not just kanji meanings) is translatable; pick from 19 bundled languages in Settings, no restart needed.
- **Settings** — dark/light theme, interface language, interface and kanji font size, stroke-animation speed, and custom paths for the database, kanji SVG data, and your localization file.
- **Polished UI** — glassmorphism styling with spring-based animations throughout (tab transitions, staggered list entrances, hover/press micro-interactions), plus a branded splash screen with no flash-of-blank-window on startup.
